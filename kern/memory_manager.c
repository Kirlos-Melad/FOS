...
inline void env_page_ws_set_entry(struct Env* e, uint32 entry_index, uint32 virtual_address){
	...
	e->ptr_pageWorkingSet[entry_index].virtual_address = va;
	e->ptr_pageWorkingSet[entry_index].empty = 0;
	e->ptr_pageWorkingSet[entry_index].sweeps = 0;

	/*//USER ENV PART
	env_page_ws_set_time_stamp(e, entry_index);
	if(isPageReplacmentAlgorithmLRULists())
	{
		LIST_REMOVE(&(e->PageWorkingSetList), &(e->ptr_pageWorkingSet[entry_index]));
		//Now: we are sure that at least the top page in the stack will be added to Active list
		//Since we left 1 empty location in the Active list when we loaded the program segments
		if (LIST_SIZE(&(e->ActiveList)) < e->ActiveListSize)
		{
			LIST_INSERT_HEAD(&(e->ActiveList), &(e->ptr_pageWorkingSet[entry_index]));
		}
		else
		{
			LIST_INSERT_HEAD(&(e->SecondList), &(e->ptr_pageWorkingSet[entry_index]));
		}
	}*/

	e->page_last_WS_index = (entry_index + 1) % e->page_WS_max_size;
}
...
inline uint32 env_page_ws_is_entry_empty(struct Env* e, uint32 entry_index){
	assert(entry_index >= 0 && entry_index < (e->page_WS_max_size));
	return (e->ptr_pageWorkingSet[entry_index].empty == 0);
}
...
void env_page_ws_print(struct Env *curenv)
{
	...
	for(i=0; i< (curenv->page_WS_max_size); i++ )
	{
		...
		uint32 sweeps = curenv->ptr_pageWorkingSet[i].sweeps;
		...
		cprintf(", used= %d, modified= %d, buffered= %d, time stamp= %x, sweeps = %d", isUsed, isModified, isBuffered, time_stamp, sweeps) ;
